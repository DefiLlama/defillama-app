...
describe('rwa perps api urls', () => {
	it('encodes contract, venue, and market ids in request paths', async () => {
		...
		expect(fetchJson.mock.calls).toEqual([
			['https://example.com/rwa-perps/market/xyz%3AMETA%2F2026?zz=16'],
			['https://example.com/rwa-perps/contract/xyz%3AMETA%2F2026?zz=16'],
			['https://example.com/rwa-perps/venue/my%20venue?zz=16'],
			['https://example.com/rwa-perps/assetGroup/US%20Equities?zz=16'],
			['https://example.com/rwa-perps/chart/venue/venue%2Fone?zz=16'],
	...
	it('builds funding history urls with optional query params', async () => {
		...
		expect(fetchJson.mock.calls).toEqual([
			['https://example.com/rwa-perps/funding/xyz%3AMETA?startTime=10&endTime=20&zz=16'],
			['https://example.com/rwa-perps/funding/xyz%3AMETA?zz=16']
		])
	})
})
