...
describe('strategy finder initial rows requests', () => {
	...
	it('does not request strategy rows before a lend token is selected', () => {
		...
		expect(captures).toEqual([{ endpoint: '/api/public/yields/strategy', queryString: null }])
	...
	it('requests strategy rows once a lend token is selected', () => {
		...
		expect(captures[0]).toMatchObject({ endpoint: '/api/public/yields/strategy' })
	...
	it('does not request long-short rows before a token is selected', () => {
		...
		expect(captures).toEqual([{ endpoint: '/api/public/yields/strategy-long-short', queryString: null }])
	...
	it('requests long-short rows once a token is selected', () => {
		...
		expect(captures[0]).toMatchObject({ endpoint: '/api/public/yields/strategy-long-short' })
