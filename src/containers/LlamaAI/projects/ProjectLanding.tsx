...
export function ProjectLanding({
	...
	return (
		<div className="mx-auto flex h-full w-full max-w-[760px] flex-col overflow-y-auto px-6 pt-6 pb-16">
			<header className="flex items-center justify-between gap-3 pb-10">
				<button
					type="button"
					onClick={() => void Router.push('/ai/projects')}
					className="-ml-2 flex items-center gap-1 rounded-md px-2 py-1 text-sm text-[#666] transition-colors hover:bg-[#f0f0f0] hover:text-[#1a1a1a] dark:text-[#919296] dark:hover:bg-[#1c1d1e] dark:hover:text-white"
				>
				...
				<div className="flex items-center gap-1">
					<button
						...
						className="flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-sm text-[#444] transition-colors hover:bg-[#f0f0f0] dark:text-[#c5c5c5] dark:hover:bg-[#1c1d1e]"
					>
			...
			<div className="flex items-center gap-3 pb-8">
				<div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-(--old-blue)/20 to-(--old-blue)/5 ring-1 ring-(--old-blue)/30 ring-inset">
					<Icon name="folder-plus" height={18} width={18} className="text-(--old-blue)" />
				</div>
				<div className="min-w-0 flex-1">
					<h1 className="truncate text-[28px] leading-tight font-semibold tracking-tight">{project.data.name}</h1>
					{project.data.description ? (
						<p className="mt-1 line-clamp-1 text-sm text-[#666] dark:text-[#919296]">{project.data.description}</p>
					) : null}
				</div>
			</div>
			...
			<form
				...
				className="group/composer relative mb-7 flex items-center gap-3 rounded-3xl border border-[#e6e6e6] bg-(--cards-bg) px-4 py-2.5 shadow-sm transition-all focus-within:border-(--old-blue)/60 focus-within:shadow-md dark:border-[#222324] dark:bg-[#161718] dark:focus-within:border-(--old-blue)/60"
			>
				<AddSourcesMenu
					projectId={projectId}
					trigger={
						<button
							...
							className="-ml-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[#666] transition-colors hover:bg-[#f0f0f0] hover:text-(--old-blue) dark:text-[#919296] dark:hover:bg-[#1c1d1e]"
						>
				...
				<textarea
					...
					className="block flex-1 resize-none self-center bg-transparent text-[15px] leading-6 text-inherit placeholder:text-[#999] focus:outline-none dark:placeholder:text-[#555]"
					style={{ height: 24 }}
				/>
				<button
					type="submit"
					...
					className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-(--old-blue) text-white transition-all hover:scale-105 hover:bg-(--old-blue)/90 disabled:scale-100 disabled:cursor-not-allowed disabled:opacity-30"
				>
					<Icon name="arrow-up" height={14} width={14} />
				</button>
			</form>

			<div role="tablist" className="-mx-1 flex items-center gap-1 border-b border-[#e6e6e6] dark:border-[#222324]">
				{(['chats', 'sources'] as const).map((id) => (
					<button
						...
						onClick={() => setTab(id)}
						className={`relative px-3 py-2.5 text-sm font-medium capitalize transition-colors focus:outline-none ${
							tab === id
								? 'text-(--old-blue)'
					...
					>
						{id}
						{tab === id ? <span className="absolute inset-x-3 -bottom-px h-0.5 rounded-full bg-(--old-blue)" /> : null}
					</button>
				))}
			</div>
								...
								{sessions.map((s) => (
									<li key={s.sessionId}>
										<button
											...
											className="group/row -mx-2 flex w-[calc(100%+1rem)] items-center justify-between gap-3 rounded-lg px-3 py-2.5 text-left transition-colors hover:bg-[#f6f6f7] dark:hover:bg-[#161718]"
										>
											<span className="min-w-0 flex-1 truncate text-sm">{s.title || 'Untitled chat'}</span>
