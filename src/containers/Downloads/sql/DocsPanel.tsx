		...
		<div className="flex flex-col gap-6">
			<header className="flex flex-col gap-1">
				...
				<p className="text-[11.5px] leading-snug text-(--text-tertiary)">
					Pragmatic idioms against DefiLlama tables — hover a technique to see what it does. Try it swaps the
					editor and runs.
				</p>
			</header>
...
const KEYWORDS = new Set([
	'ALL', 'AND', 'AS', 'ASC', 'ASOF', 'BETWEEN', 'BY', 'CASE', 'CROSS', 'CURRENT',
	'DELETE', 'DESC', 'DISTINCT', 'ELSE', 'END', 'EXCEPT', 'EXISTS', 'FALSE', 'FILTER',
	'FOLLOWING', 'FROM', 'FULL', 'GROUP', 'HAVING', 'IN', 'INNER', 'INSERT', 'INTERSECT',
	'INTO', 'IS', 'JOIN', 'LEFT', 'LIKE', 'LIMIT', 'NOT', 'NULL', 'OFFSET', 'ON', 'OR',
	'ORDER', 'OUTER', 'OVER', 'PARTITION', 'PIVOT', 'PRECEDING', 'QUALIFY', 'RIGHT',
	'ROW', 'ROWS', 'SELECT', 'SET', 'THEN', 'TRUE', 'UNBOUNDED', 'UNION', 'UNPIVOT',
	'UPDATE', 'USING', 'VALUES', 'WHEN', 'WHERE', 'WINDOW', 'WITH', 'ILIKE', 'RANGE',
	'TABLE', 'CAST', 'TRY_CAST'
])

const TYPES = new Set([
	'BIGINT', 'BOOLEAN', 'DATE', 'DECIMAL', 'DOUBLE', 'FLOAT', 'INT', 'INTEGER',
	'SMALLINT', 'TEXT', 'TIMESTAMP', 'VARCHAR', 'REAL', 'NUMERIC'
])
...
const FUNCTIONS = new Set([
	'abs', 'any_value', 'approx_count_distinct', 'approx_quantile', 'arg_max', 'arg_min',
	'array_agg', 'avg', 'coalesce', 'count', 'date_diff', 'date_part', 'date_trunc',
	'epoch', 'epoch_ms', 'first', 'first_value', 'greatest', 'json_extract',
	'json_extract_string', 'lag', 'last', 'last_value', 'lead', 'least', 'length',
	'list_aggregate', 'lower', 'max', 'median', 'min', 'nullif', 'percentile_cont',
	'quantile_cont', 'rank', 'regexp_matches', 'regexp_replace', 'round', 'row_number',
	'string_agg', 'string_split', 'strftime', 'strptime', 'substring', 'sum',
	'to_timestamp', 'trim', 'unnest', 'upper'
])
