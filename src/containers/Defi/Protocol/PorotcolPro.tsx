...
import ProtocolChart, {
	ToggleWrapper,
	Toggle as ChartToggle
} from '~/components/ECharts/ProtocolDNDChart/ProtocolChart'
...
const ChartsBody = styled.div<{ itemsCount }>`
	width: 100%;
	display: grid;
	grid-template-columns: ${({ itemsCount }) => `repeat(${itemsCount >= 3 ? 3 : itemsCount}, 1fr)`};
	grid-gap: 10px;
	margin-top: 16px;
`
...
const ProotocolDataWrapper = styled(ChartsWrapper)`
	display: flex;
	gap: 8px;
	margin-top: 0;
`
...
const chartExist: Record<keyof typeof ChartTypes, (props: any) => boolean> = {
	...
	fdv: ({ geckoId }) => !!geckoId,
	volume: (props) => props?.metrics.dexs,
	derivativesVolume: (props) => props?.metrics.derivative,
	fees: (props) => props?.metrics.fees,
	revenue: (props) => props?.metrics.fees,
	unlocks: (props) => props?.metrics.unlocks,
	activeUsers: (props) => !!props?.activeUsersId,
	newUsers: (props) => !!props?.activeUsersId,
	events: (props) => props?.hallmarks?.length > 0,
	transactions: (props) => !!props?.activeUsersId,
	gasUsed: (props) => !!props?.activeUsersId,
	medianApy: (props) => !!props?.metrics.medianApy,
	usdInflows: (props) => !!props?.metrics.inflows,
	governance: (props) => !!props?.governanceApis?.length,
	treasury: (props) => !!props?.metrics.treasury,
	bridgeVolume: (props) => props?.metrics.bridge,
	tokenVolume: ({ geckoId }) => !!geckoId,
	tokenLiquidity: ({ geckoId }) => !!geckoId,
	twitter: (props) => !!props?.twitterHandle,
	devMetrics: (props) => props?.metrics.devMetrics,
	contributersMetrics: (props) => props?.metrics.devMetrics,
	contributersCommits: (props) => props?.metrics.devMetrics,
	devCommits: (props) => props?.metrics.devMetrics,
	nftVolume: (props) => props?.metrics.nftVolume,
	staking: (props) => !!props?.historicalChainTvls['staking']?.tvl?.length,
	borrowed: (props) => !!props?.historicalChainTvls['borrowed']?.tvl?.length
}
...
function ProtocolContainer({
	...
	const [items, setItems] = useState(['tvl'])
	...
	return (
		<Layout title={title} backgroundColor={transparentize(0.6, backgroundColor)} style={{ gap: '16px' }}>
			...
			{['SyncDEX Finance', 'Avatr', 'SatoshiCoreSwap', 'Opankeswap'].includes(name) && (
			...
			<ChartsWrapper>
				<ToggleWrapper>
					{Object.entries(ChartTypes).map(([key, val]) =>
						chartExist[key]?.(chartProps) ? (
							<ChartToggle backgroundColor={backgroundColor} key={key}>
								<input
									type="checkbox"
									value={key}
									checked={items.includes(key)}
									onChange={() =>
										items.includes(key) ? setItems(items.filter((id) => id !== key)) : setItems(items.concat(key))
									}
								/>
						...
						<SortableContext items={items} strategy={rectSortingStrategy}>
							{items.map((id) => (
								<SortableItem id={id} key={id + items.length}>
									<ProtocolChart {...chartProps} enabled={{ tvl: 'false', [id]: 'true' }} name={ChartTypes[id]} />
								</SortableItem>
							))}
						</SortableContext>
					</DndContext>
