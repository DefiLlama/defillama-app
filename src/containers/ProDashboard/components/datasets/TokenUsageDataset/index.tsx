		...
		return (
			<div className="w-full p-4 h-full flex flex-col">
				...
				<div className="flex-1 min-h-[500px] flex flex-col items-center justify-center gap-4">
					...
					<button
						onClick={() => refetch()}
						className="px-4 py-2 bg-[var(--primary1)] text-white rounded hover:bg-[var(--primary1-hover)]"
					>
						Try again
					</button>
				</div>
		...
		<div className="w-full p-4 h-full flex flex-col">
			...
			{tokenSymbols.length === 1 ? (
				<div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4">
					<div className="p-3 border border-[var(--divider)]">
						<div className="text-xs text-[var(--text3)] mb-1">Total Value</div>
						<div className="text-lg font-semibold pro-text1">{formattedNum(totalAmount, true)}</div>
					</div>
					<div className="p-3 border border-[var(--divider)]">
						<div className="text-xs text-[var(--text3)] mb-1">Protocols</div>
						<div className="text-lg font-semibold pro-text1">{protocolCount}</div>
					</div>
					<div className="p-3 border border-[var(--divider)]">
						<div className="text-xs text-[var(--text3)] mb-1">Top Category</div>
					...
					<div className="p-3 border border-[var(--divider)]">
						<div className="text-xs text-[var(--text3)] mb-1">Avg per Protocol</div>
				...
				<>
					<div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
						{tokenSymbols.map((symbol) => (
							<div key={symbol} className="p-3 border border-[var(--divider)]">
								<div className="text-xs text-[var(--text3)] mb-1">{symbol.toUpperCase()} Value</div>
								<div className="text-lg font-semibold pro-text1">
								...
								<div className="mt-2">
									<div className="flex justify-between items-center mb-1">
										<span className="text-xs text-[var(--text3)]">Share</span>
									...
									<div className="h-1.5 bg-[var(--bg2)] overflow-hidden">
										<div
											className="h-full bg-[var(--primary1)] transition-all duration-300"
											style={{
												width: `${
													tokenStats && tokenStats[symbol] && totalAmount > 0
						...
						{tokenSymbols.length <= 2 && (
							<div className="p-3 border border-[var(--divider)]">
								<div className="text-xs text-[var(--text3)] mb-1">Total Combined</div>
								<div className="text-lg font-semibold pro-text1">{formattedNum(totalAmount, true)}</div>
								<div className="text-xs text-[var(--text3)]">All tokens</div>
							</div>
						)}

						{tokenSymbols.length <= 3 && (
							<div className="p-3 border border-[var(--divider)]">
								<div className="text-xs text-[var(--text3)] mb-1">Unique Protocols</div>
								<div className="text-lg font-semibold pro-text1">{protocolCount}</div>
								<div className="text-xs text-[var(--text3)]">
									{(() => {
					...
					{protocolOverlap && (
						<div className="p-3 border border-[var(--divider)] mb-4">
							<div className="text-xs text-[var(--text3)] mb-2">Protocol Distribution</div>
							<div className="flex items-center gap-4 flex-wrap">
								<div className="flex items-center gap-2">
									<div className="w-3 h-3 bg-[var(--primary1)]" />
									<span className="text-sm">
										<span className="font-semibold">{protocolOverlap.shared}</span>
										<span className="text-[var(--text3)]"> shared</span>
								...
								{tokenSymbols.map((symbol, index) => (
									<div key={symbol} className="flex items-center gap-2">
										<div
											className="w-3 h-3 border-2 border-[var(--divider)]"
											style={{
										...
										<span className="text-sm">
											...
											<span className="text-[var(--text3)]"> {symbol.toUpperCase()} only</span>
										</span>
									</div>
								))}

								<div className="ml-auto text-sm text-[var(--text3)]">Total: {protocolCount} protocols</div>
							</div>
						</div>
					)}
				</>
							...
							<tr key={headerGroup.id}>
								{headerGroup.headers.map((header) => (
									<th
										...
										className="bg-transparent font-medium px-2 py-2 border-b border-r border-[var(--divider)] last:border-r-0"
									>
