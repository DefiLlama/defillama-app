...
export function useYieldsTable({
	...
	const filteredData = React.useMemo(() => {
		...
		const selectedChainsSet =
			filters.chains && filters.chains.length > 0 ? new Set(filters.chains.map(toChainSlug)) : null
		...
		return data.filter((row) => {
			...
			if (selectedChainsSet) {
				const hasMatchingChain = row.chains?.some((chain: string) => selectedChainsSet.has(toChainSlug(chain)))
