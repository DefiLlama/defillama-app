...
export function useDimensionProtocols() {
	const { data: perpsData, isLoading: perpsLoading } = useQuery({
		...
		queryFn: () => fetchJson('/api/dynamic/adapter-metrics/perps'),
		staleTime: Infinity,
		retry: 1
	})
	...
	const { data: dexsData, isLoading: dexsLoading } = useQuery({
		...
		queryFn: () => fetchJson('/api/dynamic/adapter-metrics/dexs'),
		staleTime: Infinity,
		retry: 1
	})
	...
	const { data: aggregatorsData, isLoading: aggregatorsLoading } = useQuery({
		...
		queryFn: () => fetchJson('/api/dynamic/adapter-metrics/dex-aggregators'),
		staleTime: Infinity,
		retry: 1
	})
