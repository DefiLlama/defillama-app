...
function resolveMovingAverageLength(
	window: MetricWindow,
	arrLength: number,
): number {
	switch (window) {
...
export function useMetricData(metric: MetricConfig) {
	...
	const compare = useMemo(() => {
		...
		const format =
			metric.compare.format === "absolute" ? "absolute" : "percent";
	...
	const geckoId =
		subject.geckoId ??
		(subject.itemType === "protocol"
			? (protocols.find((p: any) => p.slug === subject.protocol)?.geckoId as
					| string
					| undefined)
			: (chains.find((c: any) => c.name === subject.chain)?.gecko_id as
					| string
					| undefined));
	...
	const {
		...
		queryKey: [
			"metric",
			...getChartQueryKey(type, itemType, item, geckoId, undefined),
		],
		...
		enabled:
			...
			((itemType === "protocol" && (!tokenTypes.includes(type) || !!geckoId)) ||
				(itemType === "chain" &&
					(!chainTokenTypes.includes(type) || !!geckoId))),
	});
	...
	const result = useMemo(() => {
		...
		const sparklineData =
			inWindow.length > 60 ? inWindow.slice(inWindow.length - 60) : inWindow;
		const lastUpdatedTs =
			inWindow.length > 0
