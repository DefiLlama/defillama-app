		...
		<div className="flex items-center rounded-2xl border border-(--sub-brand-primary) bg-(--sub-brand-primary)/20 p-4">
			<div className="flex w-full flex-col gap-4 sm:w-[200px]">
					...
					<ul className="flex flex-col gap-1.5">
						{BENEFITS.map((b) => (
							<li key={b} className="flex items-start gap-1.5">
								<Icon name="check" height={14} width={14} className="mt-0.5 shrink-0 text-(--sub-brand-primary)" />
								<span className="text-xs leading-4 text-(--sub-ink-primary) dark:text-white">{b}</span>
							</li>
						))}
					</ul>
				</div>
				<button
					onClick={onUpgrade}
					disabled={isLoading}
					className="flex h-8 w-full items-center justify-center rounded-lg bg-(--sub-brand-primary) px-3 text-xs font-medium text-white disabled:opacity-50"
				>
