	...
	const rawProcessed = chart.reduce((acc, [timestamp, data]) => {
		Object.entries(data).forEach(([chain, chainData]) => {
			Object.entries(chainData).forEach(([_protocolName, value]) => {
				if (!legend.includes(formatChain(chain))) {
					legend.push(formatChain(chain))
				}

				let finalValue = getOkValue(value)
				...
				if (!acc[`${timestamp}${chain}`]) {
					acc[`${timestamp}${chain}`] = {
						[formatChain(chain)]: finalValue,
						date: String(timestamp)
					} as IJoin2ReturnType[number]
				} else {
					acc[`${timestamp}${chain}`] = {
						[formatChain(chain)]: +acc[`${timestamp}${chain}`][formatChain(chain)] + finalValue,
						date: String(timestamp)
					} as IJoin2ReturnType[number]
				}
			})
