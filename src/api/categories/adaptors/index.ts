...
export const getChainPageData = async (type: string, chain?: string): Promise<IOverviewProps> => {
	...
	const protocolsWithSubrows = protocols.reduce((acc, protocol) => {
		...
		if (acc[protocol.parentProtocol]) {
			...
			let change_30d_abs = acc[protocol.parentProtocol].subRows.reduce(reduceSumByAttribute('change_30d_abs'), null)
			...
			mainRow.change_1m = getPercentChange(mainRow.total24h, mainRow.total24h - change_30d_abs)
