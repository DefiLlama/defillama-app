...
export const getChainsPageData = async (type: string): Promise<IOverviewProps> => {
	...
	let protocols = dataByChain.map(
		({ total24h, change_1d, change_7d, chain, change_1m, protocols, change_7dover7d, total7d, total30d }) => {
			const tvlData = getTVLData(protocolsData, chain)
			return {
				name: chain,
				displayName: chain,
...
export const joinCharts2 = (...lists: Array<[string, Array<[number, number]>]>): IJoin2ReturnType =>
	Object.values(
		lists.reduce((acc, [name, list]) => {
			list.forEach(([timestamp, value], index) => {
				let finalValue = value

				// extrapolate missing values
				if (value === 0) {
					const prev = list[index - 1]?.[1] ?? list[index - 2]?.[1]

					const next = list[index + 1]?.[1] ?? 0

					if (prev) {
						list[index][1] = prev
						finalValue = (prev + next) / (next ? 2 : 1)
					}
				}
