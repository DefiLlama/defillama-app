...
export async function getDefiChainsPageData(category: string) {
	const res: {
		chains: string[]
		protocols: LiteProtocol[]
	} = await fetchWithErrorLogging(PROTOCOLS_API).then((res) => res.json())
	...
	const chainsData = await Promise.all(
		chainsUnique.map(async (elem: string) => {
			for (let i = 0; i < 5; i++) {
				try {
					return await fetchWithErrorLogging(`${CHART_API}/${elem}`).then((resp) => resp.json())
				} catch (e) {
					console.log(elem, e)
				}
			}
	...
	const chainMcaps = await fetchWithErrorLogging(MCAPS_API, {
		method: 'POST',
		body: JSON.stringify({
...
export const getNewChainsPageData = async (category: string) => {
	const [
		...
		getDefiChainsPageData(category),
