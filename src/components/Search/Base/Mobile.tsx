			...
			{display ? (
				<>
					...
					<div className="h-full max-h-[240px] overflow-y-auto bg-(--cards-bg) rounded-b-md shadow-sm z-10 absolute left-2 right-2 top-[56px]">
						{status === 'loading' ? (
							<p className="text-(--text-primary) py-6 px-3 text-center">Loading...</p>
						) : finalResults.length ? (
							<>
		...
		<>
			{display ? (
				<>
					...
					<div className="h-full max-h-[240px] overflow-y-auto bg-(--bg-main) border border-[hsl(204,20%,88%)] dark:border-[hsl(204,3%,32%)] rounded-b-md shadow-sm z-10 absolute left-2 right-2 top-[56px]">
						{loading ? (
							<p className="text-(--text-primary) py-6 px-3 text-center">Loading...</p>
						) : finalResults.length ? (
							<>
...
const Row = ({ data, onItemClick }: { data: any; onItemClick?: (data: any) => void }) => {
	...
	return (
		<button
			...
			disabled={loading}
			className="w-full p-3 flex items-center gap-4 text-(--text-primary) cursor-pointer aria-selected:bg-(--bg-secondary) aria-disabled:opacity-50 aria-disabled:bg-(--bg-secondary)"
		>
