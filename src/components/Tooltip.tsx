...
export function Tooltip({ content, children, placement = 'top-start', className, ...props }: ITooltip) {
	...
	return (
		<Ariakit.TooltipProvider store={store}>
			...
			<Ariakit.Tooltip
				store={store}
				className="z-50 max-h-[calc(100vh-80px)] max-w-56 overflow-auto rounded-md border border-[hsl(204,20%,88%)] bg-(--bg-main) p-2 text-sm whitespace-pre-wrap data-[fullwidth=true]:max-w-[var(--popover-available-width)] lg:max-h-[var(--popover-available-height)] dark:border-[hsl(204,3%,32%)]"
				unmountOnHide
				portal
