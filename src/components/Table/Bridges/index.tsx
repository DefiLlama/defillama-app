...
export function BridgesTable({ data, searchValue = '', onSearchChange: _onSearchChange = null }) {
	...
	React.useEffect(() => {
		...
		const order = width ? (bridgesColumnOrders.find(([size]) => width >= size)?.[1] ?? defaultOrder) : defaultOrder
...
export function BridgeChainsTable({ data }) {
	...
	React.useEffect(() => {
		...
		const order = width ? (bridgeChainsColumnOrders.find(([size]) => width > size)?.[1] ?? defaultOrder) : defaultOrder
...
export function BridgesLargeTxsTable({ data }) {
	...
	React.useEffect(() => {
		...
		const order = width ? (largeTxsColumnOrders.find(([size]) => width >= size)?.[1] ?? defaultOrder) : defaultOrder
