...
export function usePullToRefresh({
	...
	const handleTouchStart = useCallback((e: TouchEvent) => {
		...
		console.log('Touch start, scrollTop:', element.scrollTop)
	...
	useEffect(() => {
		const element = elementRef.current
		if (!element || !enabled) {
			console.log('Pull to refresh not enabled:', { element: !!element, enabled })
			return
		}
		
		console.log('Adding pull to refresh listeners to element:', element)
		
		// Add debugging listeners for both touch and mouse
		const debugTouch = (e: TouchEvent) => console.log('Touch event detected:', e.type)
		const debugMouse = (e: MouseEvent) => console.log('Mouse event detected:', e.type)
		
		element.addEventListener('touchstart', handleTouchStart, { passive: false })
		element.addEventListener('touchmove', handleTouchMove, { passive: false })
		element.addEventListener('touchend', handleTouchEnd)
		element.addEventListener('touchstart', debugTouch)
		
		// Also add mouse events for desktop testing
		element.addEventListener('mousedown', debugMouse)
		element.addEventListener('click', debugMouse)
		
		return () => {
			element.removeEventListener('touchstart', handleTouchStart)
			element.removeEventListener('touchmove', handleTouchMove)
			element.removeEventListener('touchend', handleTouchEnd)
			element.removeEventListener('touchstart', debugTouch)
		}
