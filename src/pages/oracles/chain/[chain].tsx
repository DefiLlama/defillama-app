...
export const getStaticProps = withPerformanceLogging('oracles/[chain]', async ({ params }) => {
	...
	const canonicalChain = oracleRoutes.chainNameBySlug[slug(chain)]
	if (canonicalChain) {
		chain = canonicalChain
	} else {
		for (const routeChainSlug in oracleRoutes.chainNameBySlug) {
			if (routeChainSlug) return { notFound: true }
		}
	}
