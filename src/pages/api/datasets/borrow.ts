...
async function handler(req: NextApiRequest, res: NextApiResponse<BorrowPageRowsResponse | { error: string }>) {
	try {
		...
		res.setHeader('Cache-Control', jitterCacheControlHeader(CACHE_CONTROL, req.url ?? '/api/datasets/borrow'))
...
export default withApiRouteTelemetry('/api/datasets/borrow', handler)
