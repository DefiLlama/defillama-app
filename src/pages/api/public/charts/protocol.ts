...
const setSuccessCacheHeaders = (req: NextApiRequest, res: NextApiResponse<ResponseData>) => {
	res.setHeader(
		'Cache-Control',
		jitterCacheControlHeader(SUCCESS_CACHE_CONTROL, req.url ?? '/api/public/charts/protocol')
	)
}
	...
	try {
		...
		if (kind === 'adapter-breakdown') {
			const parsedRequest = parseAdapterBreakdownRequest({
				adapterType: getQueryParam(req.query.adapterType),
				protocol: getQueryParam(req.query.protocol),
				type: getQueryParam(req.query.type),
				dataType: getQueryParam(req.query.dataType)
			})
...
export default withApiRouteTelemetry('/api/public/charts/protocol', handler)
