...
async function handler(req: NextApiRequest, res: NextApiResponse) {
	try {
		...
		const upstream = await fetchWithPoolingOnServer(`${YIELDS_SERVER_URL}/chartLendBorrow/${configID}`)
		if (!upstream.ok) {
...
export default withApiRouteTelemetry('/api/public/datasets/chartLendBorrow/[configID]', handler)
