...
async function handler(req: NextApiRequest, res: NextApiResponse<YieldHalalPageResponse | { error: string }>) {
	...
	try {
		...
		res.setHeader(
			'Cache-Control',
			jitterCacheControlHeader(CACHE_CONTROL, req.url ?? '/api/public/datasets/yields/halal')
		)
...
export default withApiRouteTelemetry('/api/public/datasets/yields/halal', handler)
