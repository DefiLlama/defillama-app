...
async function handler(req: NextApiRequest, res: NextApiResponse<YieldStrategyPageResponse | { error: string }>) {
	try {
		...
		res.setHeader(
			'Cache-Control',
			jitterCacheControlHeader(CACHE_CONTROL, req.url ?? '/api/public/datasets/yields/strategy')
		)
...
export default withApiRouteTelemetry('/api/public/datasets/yields/strategy', handler)
