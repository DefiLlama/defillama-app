...
async function handler(req: NextApiRequest, res: NextApiResponse<YieldPoolsPageResponse | { error: string }>) {
	try {
		...
		res.setHeader(
			'Cache-Control',
			jitterCacheControlHeader(CACHE_CONTROL, req.url ?? '/api/public/datasets/yields/pools')
		)
...
export default withApiRouteTelemetry('/api/public/datasets/yields/pools', handler)
