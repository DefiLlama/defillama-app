...
async function handler(req: NextApiRequest, res: NextApiResponse<YieldLoopPageResponse | { error: string }>) {
	try {
		...
		res.setHeader(
			'Cache-Control',
			jitterCacheControlHeader(CACHE_CONTROL, req.url ?? '/api/public/datasets/yields/loop')
		)
...
export default withApiRouteTelemetry('/api/public/datasets/yields/loop', handler)
