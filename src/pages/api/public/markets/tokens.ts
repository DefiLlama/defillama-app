...
async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	const url = `${MARKETS_SERVER_URL}/tokens/list.json`
	try {
		...
		console.error(error)
		return res.status(502).json({ error: 'Failed to load token markets list' })
	}
}

export default withApiRouteTelemetry('/api/public/markets/tokens', handler)
