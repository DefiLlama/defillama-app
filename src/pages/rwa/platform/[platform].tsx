...
export default function RWAPage(props) {
	return (
		<Layout
			title={`${props.platform} - RWA - DefiLlama`}
			description={`${props.platform} RWA on DefiLlama. DefiLlama is committed to providing accurate data without ads or sponsored content, as well as transparency.`}
			keywords={`${props.platform}, real world assets, defi rwa rankings, rwa on chain`}
			pageName={pageName}
			canonicalUrl={`/rwa`}
		>
			<RWAOverview {...props} />
