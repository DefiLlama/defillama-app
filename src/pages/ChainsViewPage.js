...
const StackedChart = ({
  ...
  <Panel style={{ height: '100%', margin: '0.5em' }}>
...
const ChainsView = () => {
  ...
  useEffect(() => {
    const chainsCalls = chainsUnique.map(elem =>
      fetch(`https://api.llama.fi/lite/charts/${elem}`).then(resp => resp.json())
    )
  ...
  const chainColor = useMemo(() => Object.fromEntries(chainsUnique.map(chain => [chain, getRandomColor()])), [
    chainsUnique
  ])
  const chainTvls = useMemo(
    () =>
      (data.length > 0 ? chainsUnique : []).map((chainName, i) => {
