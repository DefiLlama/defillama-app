...
const TokenInfoHook = protocol => {
  ...
  useEffect(() => {
    if (tokenData.gecko_id) {
      fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${tokenData.gecko_id}&vs_currencies=${defaultCurrency}`)
        .then(async geckoResult => {
...
function ComparisonPage({ protocolA: protocolARouteParam, protocolB: protocolBRouteParam }) {
  ...
  document.title =
    tokenAValid && tokenBValid
      ? `${tokenAData.name} vs ${tokenBData.name} Price Comparison - DefiLlama`
      : 'Protocol Price Comparison - DefiLlama'
